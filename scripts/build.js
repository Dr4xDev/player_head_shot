...
const parseBindings = (raw) => {
    ...
    if (trimmed.startsWith('{')) {
        ...
        const names = inner.split(',').map((name) => name.trim()).filter(Boolean);
    ...
    return {names: [], defaultName: trimmed};
};
...
async function compileVueModule(filePath) {
    ...
    for (const entry of imports) {
        ...
        if (specifier === 'vue') {
            ...
            for (const name of bindingInfo.names) {
                vueNamedImports.add(name);
            }
            continue;
        }
        ...
        if (specifier.endsWith('.vue')) {
            ...
            if (bindingInfo.names.length) {
                moduleImportLines.push(
                    `const { ${bindingInfo.names.join(', ')} } = __require('${dependencyId}');`
                );
            }
            continue;
        }
    ...
    if (vueNamedImports.size) {
        preambleLines.push(`const { ${Array.from(vueNamedImports).sort().join(', ')} } = window.Vue;`);
    }
